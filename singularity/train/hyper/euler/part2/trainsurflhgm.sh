...
#python train.py  --version $version --gnn $gnn --gnn_layers $gnn_layer --gat_heads $gat_head --train_type='surf' --data_dir='/speedrun/cortexode-data-rp/' --model_dir='/cortexode/ckpts/exp_csrf_gnn_1/model/' --init_dir='/cortexode/ckpts/exp_csrf_gnn_1/init/' --data_name='adni'  --surf_hemi='lh' --surf_type='gm' --n_epochs=100 --n_samples=150000 --tag='csrf' --solver='rk4' --step_size=0.1 --device='gpu' 
#python train.py --model_file 'model_gm_adni_lh_csrf_v1_gnngat_layerss5_sf0.1_heads1_29epochs.pt' --version $version --gnn $gnn --gnn_layers $gnn_layer --gat_heads $gat_head --train_type='surf' --data_dir='/speedrun/cortexode-data-rp/' --model_dir='/cortexode/ckpts/exp_csrf_gnn_2/model/' --init_dir='/cortexode/ckpts/exp_csrf_gnn_2/init/' --data_name='adni'  --surf_hemi='lh' --surf_type='gm' --n_epochs=60 --start_epoch=31 --n_samples=150000 --tag='csrf' --solver='rk4' --step_size=0.1 --device='gpu' 
...
if [ "$gnn_layer" -eq 4 ]; then
    if [ "$st" == "gm" ] && [ "$sh" == "lh" ]; then
        model_file="model_gm_adni_lh_csrf_v2_gnngat_layers4_sf0.1_heads1_130epochs_euler.pt"
    elif [ "$st" == "wm" ] && [ "$sh" == "lh" ]; then
        model_file="model_wm_adni_lh_csrf_v2_gnngat_layers4_sf0.1_heads1_110epochs_euler.pt"
    elif [ "$st" == "gm" ] && [ "$sh" == "rh" ]; then
        model_file="model_gm_adni_rh_csrf_v2_gnngat_layers4_sf0.1_heads1_130epochs_euler.pt"
    elif [ "$st" == "wm" ] && [ "$sh" == "rh" ]; then
        model_file="model_wm_adni_rh_csrf_v2_gnngat_layers4_sf0.1_heads1_110epochs_euler.pt"
    fi
elif [ "$gnn_layer" -eq 3 ]; then
    if [ "$st" == "gm" ] && [ "$sh" == "lh" ]; then
        model_file="model_gm_adni_lh_csrf_v2_gnngat_layers3_sf0.1_heads1_160epochs_euler.pt"
    elif [ "$st" == "wm" ] && [ "$sh" == "lh" ]; then
        model_file="model_wm_adni_lh_csrf_v2_gnngat_layers3_sf0.1_heads1_130epochs_euler.pt"
    elif [ "$st" == "gm" ] && [ "$sh" == "rh" ]; then
        model_file="model_gm_adni_rh_csrf_v2_gnngat_layers3_sf0.1_heads1_160epochs_euler.pt"
    elif [ "$st" == "wm" ] && [ "$sh" == "rh" ]; then
        model_file="model_wm_adni_rh_csrf_v2_gnngat_layers3_sf0.1_heads1_130epochs_euler.pt"
    fi
elif [ "$gnn_layer" -eq 2 ]; then
    if [ "$st" == "gm" ] && [ "$sh" == "lh" ]; then
        model_file="model_gm_adni_lh_csrf_v2_gnngat_layers2_sf0.1_heads1_190epochs_euler.pt"
    elif [ "$st" == "wm" ] && [ "$sh" == "lh" ]; then
        model_file="model_wm_adni_lh_csrf_v2_gnngat_layers2_sf0.1_heads1_150epochs_euler.pt"
    elif [ "$st" == "gm" ] && [ "$sh" == "rh" ]; then
        model_file="model_gm_adni_rh_csrf_v2_gnngat_layers2_sf0.1_heads1_200epochs_euler.pt"
    elif [ "$st" == "wm" ] && [ "$sh" == "rh" ]; then
        model_file="model_wm_adni_rh_csrf_v2_gnngat_layers2_sf0.1_heads1_150epochs_euler.pt"
    fi
fi
...
python train.py --model_file $model_file --version $version --gnn $gnn --gnn_layers $gnn_layer --gat_heads $gat_head --train_type='surf' --data_dir='/speedrun/cortexode-data-rp/' --model_dir='/cortexode/ckpts/exp_csrf_gnn_2/model/' --init_dir='/cortexode/ckpts/exp_csrf_gnn_2/init/' --data_name='adni'  --surf_hemi=$sh --surf_type=$st --n_epochs=401 --start_epoch=$start_epoch --n_samples=150000 --tag='csrf' --solver='euler' --step_size=0.1 --device='gpu' 
