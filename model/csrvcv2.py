...
class CombinedGNN(nn.Module):
    def __init__(self, input_features=256, hidden_features=128, output_dim=3, num_classes=10, num_layers=2, gat_heads=8, use_gcn=True):
        ...
        # Output layer
        total_output_dim = output_dim + num_classes  # 3 + num_classes
        if use_gcn:
            self.layers.append(GCNConv(current_dim, total_output_dim))
        else:
            self.layers.append(GATConv(current_dim, total_output_dim, heads=1))
    ...
    def forward(self, x, edge_index):
        ...
        # Split the output into dx and class_logits
        dx = x[:, :3]  # First 3 dimensions
        class_logits = x[:, 3:]  # Remaining dimensions
