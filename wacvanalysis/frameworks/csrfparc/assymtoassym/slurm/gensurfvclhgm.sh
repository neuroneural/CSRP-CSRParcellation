...
python generateTestDiceAndSurfaces.py --model_file=$model_file --data_dir='/speedrun/cortexode-data-rp/' --model_dir='/cortexode/ckpts/hcp_vc_assym_gnn_0/model/' --parc_init_dir='/cortexode/ckpts/hcp_vc_assympredassym_test/parc_init/' --result_dir='/cortexode/ckpts/hcp_vc_assympredassym_test/result/' --model_type 'csrvc' --model_type2 'csrvc' --patience 'standard' --visualize 'yes' --version $version --gnn $gnn --gnn_layers $gnn_layer --gat_heads $gat_head --train_type='surfvc' --data_name='hcp'  --surf_hemi='lh' --surf_type='gm' --n_samples=150000 --tag='vc' --device='gpu' 
