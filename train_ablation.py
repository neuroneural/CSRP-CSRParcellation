...
def train_surf(config):
    ...
    if config.use_pytorch3d_normal == 'no':
        use_pytorch3d_normal = False
    ...
    if config.model_type == 'csrf' and config.version=='1':
        assert False, "wasn't in ablation study"
        ...
        print('version 2 is loading, single fully connected layer, no position or normal')
    ...
    elif config.model_type == 'csrf' and config.version=='4':
        print('version 4 is loading, ablating position')
        cortexode = CSRFnetV4(dim_h=C, kernel_size=K, n_scale=Q,
                        use_pytorch3d_normal=use_pytorch3d_normal,
                       sf=config.sf,
                       gnn_layers=config.gnn_layers,
                       use_gcn=use_gcn,
    ...
    elif config.model_type == 'csrf' and config.version=='5':
        print('version 5 is loading, ablating normals')
        cortexode = CSRFnetV5(dim_h=C, kernel_size=K, n_scale=Q,
                       use_pytorch3d_normal=use_pytorch3d_normal,
                       sf=config.sf,
                       gnn_layers=config.gnn_layers,
                       use_gcn=use_gcn,
